...
acc <- acc_output %>%    
  dplyr::select(mapunit1, slice, kap, acc, spat_p_theta1,spat_p_theta.5, spat_p_theta0, aspat_p_theta1,aspat_p_theta.5, aspat_p_theta0 ) %>%
  distinct() %>% 
...
acc2 <- acc  %>%
  ...
  mutate(type = case_when(
    accuracy_type %in% c("acc", "kap") ~ "0_machine",
...
overall_acc <- ggplot(aes(y = value, x = factor(accuracy_type, level = c("kap",  "acc",  "Θ  1", "Θ .5", "Θ  0")), fill = slice), data = acc2 ) + 
  geom_boxplot() +
  
  #aes(lower=mean-sd,upper=mean+sd,middle=mean,ymin=mean-3*sd,ymax=mean+3*sd),stat="identity"),
...
acc_sum_long <- acc_sum_long %>%
  ...
  mutate(theta = case_when(
    theta == 5 ~ '0.5', 
    accuracy_type == "kap" ~ "kappa",
    accuracy_type == "acc" ~ "accuracy",
    .default = theta))
...
p2 <- ggplot(aes(y = valuepc, x = factor(theta, level = c('accuracy', "kappa", "1", "0.5", "0")), fill = type), data = acc_sum_long ) +
   geom_boxplot() +
