...
write.csv(reduced_vars, file.path(fid$model_inputs0310[2], "reduced_covariate_list.csv"))
...
model_bgc <- lapply(names(bgc_pts_subzone), function(xx){
    ...
    tdat <- tdat %>% mutate(slice = factor(slice))
    tdat <- tdat %>% 
      dplyr::select(fnf, x, y, bgc_cat, mapunit1, mapunit2,                               position, transect_id, tid, slice, everything())
   
    tdat_all <- tdat[complete.cases(tdat[ , 11:length(tdat)]),]
    tdat_centre <- tdat_all %>% dplyr::filter(position %in% "Orig")
 
    #trDat_all <- inmdata_all[complete.cases(inmdata_all[ , 7:length(inmdata_all)]),]
   
    model_report(tdat_all, outDir)
...
model_out = file.path(paste(outDir, balance, sep = "/"))
if(!dir.exists(file.path(model_out ))){dir.create(file.path(model_out ))}
...
reduced.var <- read.csv(file.path (in_dir,  "reduced_covariate_list.csv"))
reduced.var <- reduced.var$x
...
#### NEED TO DETERMINE IF THE TUNING ETC IS BUILT PER BEC 


# reduce the number of variables 
  trDat <- tdat_all  %>% 
    dplyr::select(position, slice, transect_id, tid, bgc_cat, mapunit1, mapunit2, any_of(reduced.var)) %>%
