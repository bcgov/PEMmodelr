...
# check the number of points per mapunit and filter out mapunits with less than 20 actual origin points

mu_check <- tpts %>%
  dplyr::filter(position %in% "Orig")

MU_count <- mu_check %>% dplyr::count(mapunit1) %>% filter(n > 20) 
  
# remove training points with less than 20 actual points 
tpts <- tpts %>% 
  dplyr::filter(mapunit1 %in% MU_count$mapunit1)  %>%
  droplevels() 
...
# drop the columns not needed in model

mpts <- tpts %>%
    dplyr::select(-c(id,order, point_type,transect_id, observer, transition, data_type, stuc_stage, stuc_mod, date_ymd, time_hms, edatope, comments, photos)) %>%
    dplyr::select(fnf, x, y, bgc_cat, mapunit1, mapunit2, position,  tid, slice, any_of(mcols))
