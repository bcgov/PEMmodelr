...
# Decide what to do with Forest/ non ofrwst split? 
...
saveRDS(mcols, file.path(fid$model_inputs0310[2], "covariate_list.rds"))

# drop the columns not needed in model

mpts <- tpts %>%
    dplyr::select(-c(id,order, point_type,transect_id, observer, transition, data_type,mapunit1, mapunit2, stuc_stage, stuc_mod, date_ymd, time_hms, edatope, comments, photos)) %>%
    dplyr::select(fnf, x, y, bgc_cat, target, target2, position,  tid, slice, any_of(mcols))
