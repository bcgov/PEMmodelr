...
for(i in bgcs){
  ...
  # th = levels(theta_vals)[1]
     ...
     allthetas <- allthetas %>% mutate(slice = k)
  
  } # end of slice loop 
  
  
   acc <- theta_acc %>%
      select(slice, theta, aspat_paf_theta0, aspat_paf_theta.5,
            aspat_paf_theta1,spat_paf_theta0, spat_paf_theta.5,
            spat_paf_theta1 ) %>%
      distinct()
  
  write.csv(acc, file.path(bgcs, "compiled_theta_results.csv"))
