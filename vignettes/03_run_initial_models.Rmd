...
for(i in bgcs){
  ...
  slices <- as.factor(list.files(i))

  theta_acc <- foreach::foreach(k = levels(slices),.combine = rbind) %do% {

  k = levels(slices)[1]
  pred_all <- readRDS(file.path(i, k))
  ...
  allthetas <- foreach::foreach(th = levels( theta_vals ),.combine = rbind) %do% {
    ...
    tacc <- acc_metrics(pred_all, fuzzmatrx = fuzz_matrix, theta = as.numeric(th)) 
    tacc <- tacc %>% 
      mutate(theta = th)
    print("Generated acc metrics for theta:",i)
  ...
  acc <- read.csv(file.path(fid$model_draft[2], i,"acc_base_results.csv"))
  
  acc <- acc %>% select(aspat_paf_theta0, aspat_paf_theta.5,
  ...
  acc1 <- acc %>% 
      summarise(across(aspat_paf_theta0:  spat_paf_theta1, mean)) %>% 
    gather(.)%>%
    mutate(type = str_split_fixed(acc1$key, "_paf_", 2)[,1],
           theta = str_split_fixed(acc1$key, "_paf_", 2)[,2]) %>%
    select(-key)
  
  acc2 <- acc1 %>%
    spread(., key = theta, value = value)%>%
    select(type, theta1, theta.5, theta0)
  
  # check 1: ARE VALES OF THETA0 ABOVE 0.65
  ...
  library(stringr)
      ...
      str_split_fixed(acc1$key, "_", 2)[,1]
                                
                      gsub(acc1$key, pattern = "_paf", value = TRUE )
  ...
  model_outputs <- list.files(fid$model_draft[2], recursive = TRUE, pattern = "acc_base_results.csv")
